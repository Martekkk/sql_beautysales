...
-- Produkty z ryzykiem przeterminowania
-- Produkty z brakiem sprzedaży i dużymi zapasami (powyżej 50 sztuk)
SELECT 
    p.product_name,
    i.stock as current_stock,
    COALESCE(SUM(oi.quantity), 0) as total_sold,
    'BRAK SPRZEDAŻY - PRZECENA' as action
FROM products p
JOIN inventory i ON p.product_id = i.product_id
LEFT JOIN order_items oi ON p.product_id = oi.product_id
...
-- Produkty z bardzo niską sprzedażą w stosunku do zapasów (rotacja < 0.1)
SELECT 
    p.product_name,
    i.stock as current_stock,
    COALESCE(SUM(oi.quantity), 0) as total_sold,
    ROUND(COALESCE(SUM(oi.quantity), 0) / i.stock, 2) as turnover_ratio,
    'RYZYKO PRZETERMINOWANIA - BUNDLING' as action
FROM products p
JOIN inventory i ON p.product_id = i.product_id
LEFT JOIN order_items oi ON p.product_id = oi.product_id
GROUP BY p.product_name, i.stock
HAVING COALESCE(SUM(oi.quantity), 0) > 0 
   AND (COALESCE(SUM(oi.quantity), 0) / i.stock) < 0.1
ORDER BY turnover_ratio ASC;

-- Produkty dołączane jako upominek przy zakupach
-- 1. Lista produktów do przeceny
